...
class M5Data():
    def __init__(
        ...
        """
        Params:
            - ``n_days``: Number of hold-out days for testing (28 by default).
        """
        self.n_days = n_days

        # date	wm_yr_wk    weekday	wday	month	year    d	event_name_1	event_type_1	event_name_2	event_type_2	snap_CA	snap_TX	snap_WI
        self.calendar           = pd.read_csv(calendar_path, parse_dates=True)
    ...
    def get_dates(self, df: pd.DataFrame, real = True):
        if not real:
            return list(map(int, [col.split('_')[1] for col in df.index] )) 
        return self.calendar.set_index('d').loc[df.index, 'date']
    ...
    def get_train_val_ts(self, 
        ...
        ### INPUT
            - ``ts_id``: Time Series item_id ("HOBBIES_1_001_CA_1")
        ...
        assert start_train < N - self.n_days * n_splits 
        #ts = ts.iloc[:, start_train:].sum()

        for k in reversed(range(n_splits)):
            start_cv    = N - self.n_days * (k+1)
            ts_train= ts.iloc[:, start_train:(start_cv)].sum()
            ts_cv   = ts.iloc[:, start_cv:(start_cv + self.n_days)].sum()
    ...
    def get_test_ts(self):
        return self.sales_test.iloc[:, -self.n_days:].sum()
